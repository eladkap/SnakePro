class Snake {
  ...
  isInFrame() {
    if (
      this.head_pos.x < FRAME_X ||
      this.head_pos.x > FRAME_X + FRAME_WIDTH - this.w
    ) {
      this.stop();
      return false;
    }
    if (
      this.head_pos.y < FRAME_Y ||
      this.head_pos.y > FRAME_Y + FRAME_HEIGHT - this.w
    ) {
      this.stop();
      return false;
    }
  ...
  update() {
    ...
    if (this.tail.length > 0) {
      this.tail[this.tail.length - 1] = createVector(
        this.head_pos.x,
        this.head_pos.y
      );
    }
  ...
  collideHead(entity) {
    var d = dist(
      this.head_pos.x + this.w / 2,
      this.head_pos.y + this.w / 2,
      entity.pos.x + entity.w / 2,
      entity.pos.y + entity.w / 2
    );
    return d < this.w;
  }
  ...
  collideTail(entity) {
    for (var i = 0; i < this.tail.length; i++) {
      var d = dist(
        this.tail[i].x + this.w / 2,
        this.tail[i].y + this.w / 2,
        entity.pos.x + entity.w / 2,
        entity.pos.y + entity.w / 2
      );
      if (d < this.w) {
